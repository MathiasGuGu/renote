...
class SimpleJobWorkers {
  async start() {
    console.log("Starting simple job workers...");

    // Register workers for different job types
    simpleJobQueue.registerWorker(
      "generate-questions",
    ...
    console.log("Simple job workers started successfully");
  }

  async stop() {
    simpleJobQueue.stopProcessing();
    console.log("Simple job workers stopped");
  }

  private async handleGenerateQuestions(data: GenerateQuestionsJobData) {
  ...
  private async handleSyncNotionAccount(data: SyncNotionAccountJobData) {
  ...
  private async handleCleanupOldQuestions(data: CleanupOldQuestionsJobData) {
  ...
  // Helper method - same as in workers.ts
...
export const simpleJobWorkers = new SimpleJobWorkers();
